...
class flyerMonitor:
  ...
  def start(self, procut_id, topic):
    streaming = Streaming()
    streaming.start()
    ep = streaming.get_endpoint(procut_id, topic)
    while True:
      try:
        ep.wait_any()
  ...
  def show_boards():
    for b in boards:
      ...
      bid = sorted(b['bids'], key=itemgetter('price'), reverse=True)
      for bid in bids:
        bid['attr'] = attr['BUY']
  ...
  def show_executions(self, executions):
    for e in executions:
      ...
      msg+= "{volume_bar_attr} {volume:5.1f}|{volume_bar:<14}{bar_attr}"
      print(msg.format(**d))
